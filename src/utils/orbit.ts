...
// Pasamos los miembros como parámetro en lugar de importarlos directamente
export function generateOrbits(members: any[]): Orbit[] {
	let memberIndex = 0;
	...
	return orbitConfigs.map((config) => {
		...
		const items: OrbitItem[] = orbitMembers.map((member, i) => ({
			// Astro resuelve las imagenes en 'avatar', si es URL externa usara avatarURL
